<div align="center">

# Hi 👋 I'm Oussama Ksantini

### Software Engineer | Full Stack Developer | AWS Cloud Enthusiast

Building scalable cloud-native applications with Java, Spring Boot, React, Docker & AWS

<br/>

<a href="https://www.linkedin.com/in/oussama-ksantini">
  <img src="https://skillicons.dev/icons?i=linkedin" height="50"/>
</a>

<a href="https://oussamaksantini.com">
  <img src="https://skillicons.dev/icons?i=webflow" height="50"/>
</a>

<a href="mailto:oussamaksantini27@gmail.com">
  <img src="https://skillicons.dev/icons?i=gmail" height="50"/>
</a>

</div>

---

# 👨‍💻 About Me

I'm a Software Engineer passionate about building secure, scalable and cloud-native applications.

- 🔭 Currently building production-ready applications on AWS
- ☁️ Working with AWS EC2, RDS, S3, CloudFront and ACM
- 💻 Full Stack Developer using Spring Boot and React
- 🐳 Deploying applications using Docker and GitHub Actions
- 📚 Studying AWS Cloud Practitioner & Solutions Architect
- 🎯 Goal: Become a Senior Software & Cloud Engineer

---

# 🚀 Featured Projects

## 🔐 SecureAuth Cloud

Enterprise Authentication Platform deployed on AWS.

### Features

- JWT Authentication
- Refresh Tokens
- Email Verification
- Password Reset
- Role-Based Access Control
- Dockerized Deployment
- GitHub Actions CI/CD
- AWS EC2
- AWS RDS PostgreSQL
- AWS S3
- AWS CloudFront
- AWS ACM SSL

### Repository

https://github.com/oussemakessentini/secureauth-cloud

---

## 🌐 Personal Portfolio Platform

Modern portfolio management system built with Spring Boot and React.

### Features

- Project Management
- Education Management
- Certificate Management
- Experience Management
- Secure Admin Dashboard
- Image Upload Support
- Cloud Deployment

### Website

https://oussamaksantini.com

---

# 🛠️ Tech Stack

## ⚙️ Backend

<div align="left">
<img src="https://skillicons.dev/icons?i=java,spring,maven" />
</div>

## 🎨 Frontend

<div align="left">
<img src="https://skillicons.dev/icons?i=react,js,html,css,bootstrap" />
</div>

## 🗄️ Databases

<div align="left">
<img src="https://skillicons.dev/icons?i=postgres,mysql,redis" />
</div>

## ☁️ Cloud & DevOps

<div align="left">
<img src="https://skillicons.dev/icons?i=aws,docker,githubactions,linux,git,nginx" />
</div>

## 🧰 Tools

<div align="left">
<img src="https://skillicons.dev/icons?i=idea,vscode,postman" />
</div>

---

# ☁️ AWS Experience

```text
EC2          ✅
RDS          ✅
S3           ✅
CloudFront   ✅
ACM SSL      ✅
Docker       ✅
GitHub CI/CD ✅
Linux        ✅
Nginx        ✅
```

---

# 🏆 GitHub Achievements

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=oussemakessentini&theme=tokyonight&no-frame=true&row=1&column=6"/>

</div>

---

# 🎯 2026 Goals

- AWS Cloud Practitioner
- AWS AI Practitioner
- AWS Solutions Architect Associate
- Build Multiple Production AWS Projects
- Contribute to Open Source

---

# 📫 Contact Me

<div align="left">

📧 oussamaksantini27@gmail.com

🌐 https://oussamaksantini.com

💼 https://www.linkedin.com/in/oussama-ksantini

</div>

---

<div align="center">

### Thanks for visiting my profile ⭐

</div>
